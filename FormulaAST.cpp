...
public:
    ...
    virtual double Evaluate(FormulaEvaluator) const = 0;
...
public:
    ...
    double Evaluate(FormulaEvaluator func) const override {
        double lhsValue = lhs_->Evaluate(func);
        double rhsValue = rhs_->Evaluate(func);
...
public:
    ...
    double Evaluate(FormulaEvaluator func) const override {
        double operandValue = operand_->Evaluate(func);
...
public:
    explicit CellExpr( Position* cell)
        : cell_(cell) {
    }
...
public:
    ...
    double Evaluate(FormulaEvaluator ) const override {
        return value_;
    }
...
double FormulaAST::Execute(FormulaEvaluator func) const {
